...
export function buildExplainerPlan(assets: Asset[], fs: FinancialStatements): BeatPlan[] {
  ...
  if (has("pl-ratios"))
    plan.push({
      ...
      section: "損益",
      focus:
        "粗利率・営業利益率・純利益率の水準が、この会社の費用構造について示す事実を一段踏み込んで読み解く（例：原価率が小さい＝売上に対し直接費用の比重が小さい収益構造）。定義の言い換えで終わらせない。売買の含意や将来予測は出さない。",
    });
  if (has("rev-trend"))
    plan.push({
      visualRef: "rev-trend",
  ...
  if (has("bs-proportion"))
    plan.push({
      ...
      section: "財務",
      focus:
        "比例縮尺の貸借対照表。資産＝負債＋純資産の恒等式に触れたうえで、純資産ブロックの厚み（自己資本でどれだけ賄われているか）が資本構成について示す事実を読み解く。",
    });
  if (has("bs-ratios"))
    plan.push({
      visualRef: "bs-ratios",
